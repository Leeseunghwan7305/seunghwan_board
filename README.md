...
공개 배포 시 서버 키로 모든 방문자 사용량이 청구되는 걸 막기 위해, **사용자가 자기 OpenAI 키를 직접 등록**하는 BYOK(Bring Your Own Key)를 구현했습니다.

- **흐름** — 프론트에서 키를 입력하면 브라우저 `sessionStorage`에만 저장되고, 요청마다 `X-OpenAI-Key` 헤더로 전송. 백엔드는 그 키로 **해당 요청만** OpenAI를 호출합니다.
- **키를 서버에 저장하지 않음** — DB·파일·로그 어디에도 남기지 않고 요청 처리 중에만 사용. (모듈 전역 클라이언트를 제거하고 요청마다 키로 클라이언트를 생성)
- **게이팅** — 키 미등록 시 다른 화면은 잠기고 "키 등록"만 노출. 로컬 개발에선 서버 env 키로 폴백(`get_openai_key`).
- 관련 코드 — `app/deps.py`(헤더 의존성) · `embed.py`/`answer.py`(키 주입) · `frontend/lib/key.ts`(sessionStorage).

> 설계 포인트: "사용자 키를 서버에 저장하지 않는 BYOK"는 **비용 안전 + 보안 설계**를 동시에 보여주는 부분입니다.
